...
GARDIR ?= ../../../gar
GARPKGDIR ?= ../..
PKGGROUPS ?= bio freedesktop gnome gnu gnualpha gnustep  gstreamer  other xorg libs tools dev external
...
fetch: pre-everything custom-pre-everything $(COOKIEDIR) $(DOWNLOADDIR) $(WORKDIR) $(addprefix dep-$(GARDIR)/,$(FETCHDEPS)) pre-fetch custom-pre-fetch $(addprefix $(DOWNLOADDIR)/,$(ALLFILES)) post-fetch custom-post-fetch
	$(DONADA)
...
clean:
	...
	rm --preserve-root=all -rf $(DOWNLOADDIR) $(COOKIEDIR) $(LOGDIR) $(WORKSRC) $(WORKDIR) $(EXTRACTDIR) $(SCRATCHDIR) $(SCRATCHDIR)-$(COOKIEDIR) *~
